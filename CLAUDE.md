...
**A search must stay around 500 tokens.** The reference implementation this was
built against (IBM's wxo-docs) returns whole matched sections and burns 10k+ per
search. Every change is measured against that budget; `search_docs` returns
snippets and `read_doc` is a separate call *because* of it. If a change makes
search verbose, it has broken the point of the project.
